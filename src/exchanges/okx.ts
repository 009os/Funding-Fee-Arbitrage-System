//this file contains classes of different exchanges where we have body of all functions required
...
export class Okx {
  ...
  async getTickSize(instId: string): Promise<string | null> {
    const endpoint = '/api/v5/public/instruments';
    const url = `${this.baseUrl}${endpoint}`;
    const params = { instType: 'SWAP' };

    try {
      console.log(`Fetching tick size for instrument: ${instId}`);
      ...
      if (response.data.code === '0') {
        ...
        if (instrument) {
          ...
          console.error(`Instrument ${instId} not found.`);
          return null;
        }
      } else {
        console.error(`Error fetching instruments: ${response.data.msg}`);
        return null;
      }
    } catch (error: any) {
      console.error(`Error fetching tick size: ${error.message}`);
      return null;
    }
  }
  async placeBboOrder(
    ...
    const contractValue = parseFloat(okxContractVal as string);
    size /= contractValue;
    ...
    try {
      ...
      if (orderResponse && orderResponse.code === '0') {
        ...
        return orderResponse; // Full response
      } else {
        console.error(
          `Error placing order: ${JSON.stringify(orderResponse, null, 4)}`,
        );
  ...
  async getOrderStatus(ordId: string, instId: string): Promise<string | null> {
    const endpoint = '/api/v5/trade/order';
    const params = { instId, ordId };
    const queryString = new URLSearchParams(params).toString(); // Encode the query string
    const url = `${this.baseUrl}${endpoint}?${queryString}`; // Include query string in URL
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (data.code === '0') {
          const orderState = data.data[0]?.state;
      ...
      if (error.response) {
        console.error(
          `Response Content: ${JSON.stringify(error.response.data, null, 4)}`,
        );
      }
      return 'UNKNOWN';
    }
  ...
  async cancelOrderById(ordId: string, instId: string): Promise<any> {
  ...
  async printPositionsForSymbol(instId: string): Promise<number | null> {
    ...
    try {
      ...
      if (response.status === 200 && response.data.code === '0') {
        ...
        let crossPosition: number | null = null;
          ...
          if (position.mgnMode === 'cross') {
            ...
            if (contractInfo && contractInfo.contract_value !== 'N/A') {
              const contractValue = parseFloat(
                contractInfo.contract_value as string,
              );
  ...
  async getContractInfo(instId: string): Promise<ContractInfo> {
