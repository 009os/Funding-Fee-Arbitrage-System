/* eslint-disable prettier/prettier */
import { Logger } from '@nestjs/common';
import { JobDataDto } from './dto/jobData.dto';
import { Binance } from 'src/exchanges/binance';
import { Bybit } from 'src/exchanges/bybit';
import { Okx } from 'src/exchanges/okx';
import { CryptoExchangeType, JobStatus } from '@prisma/client';
import { getBullMqRedisClient } from 'src/utils/getRedisClient';
import { REDIS_KEYS } from 'src/constants/constants';

function isOrderCanceledOkx(response: any): boolean {
  if (response && response.code === '0') {
    const dataArray = response.data || [];
    return dataArray.some((data: any) => data.sCode === '0');
  }
  return false;
}

function isOrderCanceledBybit(response: any): boolean {
  if (response && response.orderId) {
    console.log(`Order Canceled Successfully:`, response);
    return true;
  }
  return false;
}

function isOrderCanceledBinance(response: any): boolean {
  if (response && response.status === 'CANCELED') {
    console.log(`Order Cancelled:`, response);
    return true;
  }
  return false;
}


type UserConfig = {
  symbols: Record<string, string>;
  buyExchange: CryptoExchangeType;
  sellExchange: CryptoExchangeType;
  totalQuantity: number;
  minSlotQuantity: number;
  baseSymbol: string;
  assetForExchange1: string;
  assetForExchange2: string;
};

export class OrderExecutor {
  private readonly redisClient = getBullMqRedisClient();
 

  constructor(
    private readonly jobData: JobDataDto,
    private readonly longExchange,
    private readonly shortExchange,
    private readonly jobId: string,
  ) {}

  private getUserConfig(): UserConfig {
    const {
      symbol: baseSymbol,
      longExchange,
      shortExchange,
      tickQuantity: minSlotQuantity,
      quantity: totalQuantity,
      marketAssetLong: assetForExchange1,
      marketAssetShort: assetForExchange2,
    } = this.jobData;

    const symbols = {
      [this.jobData.longExchange]: `${baseSymbol}${assetForExchange1}`,
      [this.jobData.shortExchange]: `${baseSymbol}${assetForExchange2}`,
    };

    return {
      symbols,
      buyExchange: longExchange,
      sellExchange: shortExchange,
      totalQuantity: totalQuantity,
      minSlotQuantity: minSlotQuantity,
      baseSymbol: baseSymbol,
      assetForExchange1: assetForExchange1,
      assetForExchange2: assetForExchange2,
    };
  }

  async execute(): Promise<{
    processedQuantity: number;
    jobStatus: JobStatus;
  }> {
    const config = this.getUserConfig();

    Logger.log(`${JSON.stringify(this.jobData)}`);

    const buyApi = this.longExchange;
    const sellApi = this.shortExchange;

    

    let bybitPricePrecision = null;
    if(buyApi instanceof Bybit || sellApi instanceof Bybit){
      const symbolToMonitor = config.symbols[config.buyExchange] || config.symbols[config.sellExchange];
    
      // Start WebSocket if not already connected
      if (buyApi instanceof Bybit) {
        await buyApi.connectWebSocket(symbolToMonitor);
      }
      if (sellApi instanceof Bybit) {
        await sellApi.connectWebSocket(symbolToMonitor);
      }
  
      
    
    while (bybitPricePrecision === null) {
    try {

      if(buyApi instanceof Bybit){
        bybitPricePrecision = await buyApi.getPricePrecision(config.symbols[config.buyExchange]);
            if (bybitPricePrecision) {
              console.log(`Price precision for Bybit (${config.symbols[config.buyExchange]}): ${bybitPricePrecision}`);
            } else {
              throw new Error('Unable to fetch price precision for Bybit.');
            }
          } 

        if(sellApi instanceof Bybit){
          bybitPricePrecision = await sellApi.getPricePrecision(config.symbols[config.sellExchange]);
            if (bybitPricePrecision) {
              console.log(`Price precision for Bybit (${config.symbols[config.sellExchange]}): ${bybitPricePrecision}`);
            } else {
              throw new Error('Unable to fetch price precision for Bybit.');
            }
          }
      }
     catch (error) {
      console.error(`Error fetching Bybit price precision: ${error}`);
      console.log('Retrying to fetch Bybit price precision...');
      await new Promise((resolve) => setTimeout(resolve, 2000)); // Retry after 2 seconds
    }
  }}
  let okxContractVal = null;
  if(buyApi instanceof Okx || sellApi instanceof Okx){
  while (okxContractVal === null) {
    try {
      
      if(buyApi instanceof Okx){
        const buyInstId = `${config.baseSymbol}-${config.assetForExchange2}-SWAP`;
        const contractInfo = await buyApi.getContractInfo(buyInstId);
        okxContractVal = parseFloat(contractInfo.contract_value as string);
        // okxContractVal = await buyApi.getContractInfo(buyInstId);
            if (okxContractVal) {
              console.log(`okxContractVal: (${okxContractVal})`);
            } else {
              throw new Error('Unable to fetch okxContractVal.');
            }
          } 

        if(sellApi instanceof Okx){
          const sellInstId = `${config.baseSymbol}-${config.assetForExchange1}-SWAP`;
          const contractInfo = await sellApi.getContractInfo(sellInstId);
          okxContractVal = parseFloat(contractInfo.contract_value as string);
            if (okxContractVal) {
              console.log(`okxContractVal: (${okxContractVal})`);
            } else {
              throw new Error('Unable to fetch okxContractVal.');
            }
          }
      }
     catch (error) {
      console.error(`Unable to fetch okxContractVal: ${error}`);
      console.log('Retrying ...');
      await new Promise((resolve) => setTimeout(resolve, 2000)); // Retry after 2 seconds
    }}
  }

    let count = 0;
    // Calculate the total number of iterations
    const totalIterations = config.totalQuantity / (config.minSlotQuantity * 2);
    Logger.debug(
      `Total Iterations for job id ${this.jobId}: ${totalIterations}`,
    );

    //tracker to track order statuses
    const orderTracker = {
      buy: {
        id: null as string | null,
        status: 'UNKNOWN' as
          | 'UNKNOWN'
          | 'FILLED'
          | 'LIVE'
          | 'CANCELED'
          | 'PARTIALLYFILLED'
          | 'PARTIALLY_FILLED'
          | 'NEW'
          | 'EXPIRED'
          |'NULL'
          | 'CANCELLED',
      },
      sell: {
        id: null as string | null,
        status: 'UNKNOWN' as
          | 'UNKNOWN'
          | 'FILLED'
          | 'LIVE'
          | 'CANCELED'
          | 'PARTIALLYFILLED'
          | 'PARTIALLY_FILLED'
          | 'NEW'
          | 'EXPIRED'
          |'NULL'
          | 'CANCELLED',
      },
    };

    //main logic
    while (count < totalIterations) {
      const isJobStopped = await this.redisClient.sismember(
        REDIS_KEYS.FUNDING_FEE_ARBITRAGE_STOP,
        this.jobId,
      );

      if (isJobStopped) {

        await this.redisClient.srem(
          REDIS_KEYS.FUNDING_FEE_ARBITRAGE_STOP,
          this.jobId,
        );
        // TODO: CHECK FOR QUANTITY FILLED CORRECT LOGIC
        return {
          jobStatus: JobStatus.STOPPED,
          processedQuantity: count * config.minSlotQuantity * 2,
        };
      }

      console.log('#'.repeat(20));
      console.log(
        `Iteration: ${count + 1} | Total Iterations: ${totalIterations}`,
      );
      console.log('#'.repeat(20));
      const partiallyFilledCounter = { BUY: 0, SELL: 0 };

      const orderQuantity = config.minSlotQuantity;
      console.log(`Placing Order Quantity: ${orderQuantity}`);

// Place BUY order
let buyOrderId: string | null = null;
let buyretryCount = 0;
const buymaxRetries = 10;

while (!buyOrderId) {
  buyretryCount++;
    console.log(`Attempt ${buyretryCount} to place BUY order...`);
    

    // Try for Binance
    if (buyApi instanceof Binance) {
      try {
        const buyResponse = await buyApi.placeOrderBinance(
          config.symbols[config.buyExchange],
          'BUY',
          orderQuantity,
        );

        buyOrderId = buyResponse?.clientOrderId || null;
      } catch (error) {
        console.error(`Binance BUY order error: ${error}`);
      }
    }

    // Try for Bybit
    else if (buyApi instanceof Bybit) {
      try {
        const buyResponse = await buyApi.placeLimitOrder(
          config.symbols[config.buyExchange],
          'Buy',
          orderQuantity,
          bybitPricePrecision,
        );

        buyOrderId = buyResponse?.result?.orderId || null;
      } catch (error) {
        console.error(`Bybit BUY order error: ${error}`);
      }
    }
    // Try for OKX
    else if (buyApi instanceof Okx) {
      try {
        const buyInstId = `${config.baseSymbol}-${config.assetForExchange2}-SWAP`;
        const buyResponse = await buyApi.placeBboOrder(
          buyInstId,
          'buy',
          orderQuantity,
          okxContractVal,
        );

        buyOrderId = buyResponse.data[0]?.ordId || null;
      } catch (error) {
        console.error(`OKX BUY order error: ${error}`);
      }
    }
    if (buyOrderId) {
      console.log(`BUY Order ID: ${buyOrderId}`);
      orderTracker.buy.id = buyOrderId; // Update buy order ID
      orderTracker.buy.status = 'UNKNOWN'; // Set status to UNKNOWN
    }
  // Check retries and terminate if max retries are exceeded
   if (buyretryCount >= buymaxRetries && !buyOrderId) {
    console.error('Exceeded maximum retry attempts for BUY order. Terminating script.');
    throw new Error('BUY order placement failed after maximum retries.');
  }
  // Wait before retrying
   if (!buyOrderId) {
    console.log('Retrying BUY order...');
    await new Promise(res => setTimeout(res, 2000));
  }
}
// Place SELL order
let sellOrderId: string | null = null;
let sellretryCount = 0;
const sellmaxRetries = 10;

while (!sellOrderId) {
  sellretryCount++;
    console.log(`Attempt ${sellretryCount} to place SELL order...`);

    // Try for Binance
    if (sellApi instanceof Binance) {
      try {
        const sellResponse = await sellApi.placeOrderBinance(
          config.symbols[config.sellExchange],
          'SELL',
          orderQuantity,
        );

        sellOrderId = sellResponse?.clientOrderId || null;
      } catch (error) {
        console.error(`Binance sell order error: ${error}`);
      }
    }

    // Try for Bybit
    else if (sellApi instanceof Bybit) {
      try {
        const sellResponse = await sellApi.placeLimitOrder(
          config.symbols[config.sellExchange],
          'Sell',
          orderQuantity,
          bybitPricePrecision,

        );

        sellOrderId = sellResponse?.result?.orderId || null;
      } catch (error) {
        console.error(`Bybit SELL order error: ${error}`);
      }
    }
    // Try for OKX
    else if (sellApi instanceof Okx) {
      try {
        const sellInstId = `${config.baseSymbol}-${config.assetForExchange2}-SWAP`;
        const sellResponse = await sellApi.placeBboOrder(
          sellInstId,
          'sell',
          orderQuantity,
          okxContractVal,
        );

        sellOrderId = sellResponse.data[0]?.ordId || null;
      } catch (error) {
        console.error(`OKX SELL order error: ${error}`);
      }
    }
    if (sellOrderId) {
      console.log(`SELL Order ID: ${sellOrderId}`);
      orderTracker.sell.id = sellOrderId; // Update buy order ID
      orderTracker.sell.status = 'UNKNOWN'; // Set status to UNKNOWN
    }
  // Check retries and terminate if max retries are exceeded
   if (sellretryCount >= sellmaxRetries && !sellOrderId) {
    console.error('Exceeded maximum retry attempts for sell order. Terminating script.');
    throw new Error('SELL order placement failed after maximum retries.');
  }
  // Wait before retrying
   if (!sellOrderId) {
    console.log('Retrying SELL order...');
    await new Promise(res => setTimeout(res, 2000));
  }
}

      console.log('Order Tracker:', orderTracker);

      const activeStatuses = new Set(['NEW', 'LIVE']);
      const cancelStatuses = new Set(['CANCELED', 'EXPIRED', 'CANCELLED']);
      const filledStatus = 'FILLED';
      const partially_filled_status = new Set([
        'PARTIALLY_FILLED',
        'PARTIALLYFILLED',
      ]);

      //monitoring order status
      while (true) {
        await new Promise((resolve) => setTimeout(resolve, 2000));

        const buyOrderId = orderTracker.buy.id!;
        const sellOrderId = orderTracker.sell.id!;

        try {
          // Fetch current BUY order status
          let buyStatusResponse;
          if (buyApi instanceof Binance) {
            try {
              buyStatusResponse = await buyApi.fetchOrderStatusBinance(
                buyOrderId,
                config.symbols[config.buyExchange],
              );
            } catch (error) {
              console.error(`Error fetching BUY order status: ${error}`);
            }
          } else if (buyApi instanceof Bybit) {
            try {
              buyStatusResponse = await buyApi.fetchOrderStatus(
                buyOrderId,
                config.symbols[config.buyExchange],
              );
            } catch (error) {
              console.error(`Error fetching BUY order status: ${error}`);
            }
          } else if (buyApi instanceof Okx) {
            try {
              buyStatusResponse = await buyApi.getOrderStatus(
                buyOrderId,
                `${config.baseSymbol}-${config.assetForExchange2}-SWAP`,
              );
            } catch (error) {
              console.error(`Error fetching BUY order status: ${error}`);
            }
          }
          // Extract and update the BUY order status
          const buyStatus = buyStatusResponse?.toUpperCase() || 'UNKNOWN';

          orderTracker.buy.status = buyStatus;

          console.log(`Updated Buy Status: ${orderTracker.buy.status}`);

          // Fetching current SELL order status
          let sellStatusResponse;
          if (sellApi instanceof Binance) {
            try {
              sellStatusResponse = await sellApi.fetchOrderStatusBinance(
                sellOrderId,
                config.symbols[config.sellExchange],
              );
            } catch (error) {
              console.error(`Error fetching SELL order status: ${error}`);
            }
          } else if (sellApi instanceof Bybit) {
            try {
              sellStatusResponse = await sellApi.fetchOrderStatus(
                sellOrderId,
                config.symbols[config.sellExchange],
              );
            } catch (error) {
              console.error(`Error fetching SELL order status: ${error}`);
            }
          } else if (sellApi instanceof Okx) {
            try {
              sellStatusResponse = await sellApi.getOrderStatus(
                sellOrderId,
                `${config.baseSymbol}-${config.assetForExchange2}-SWAP`,
              );
            } catch (error) {
              console.error(`Error fetching SELL order status: ${error}`);
            }
          }

          // Extract and update the SELL order status
          const sellStatus = sellStatusResponse?.toUpperCase() || 'UNKNOWN';
          orderTracker.sell.status = sellStatus;

          console.log(`Updated Sell Status: ${orderTracker.sell.status}`);

          //scenario 1  dealing with partially fills:
          if (partially_filled_status.has(orderTracker.buy.status)) {
            partiallyFilledCounter.BUY += 1;
            console.log(
              `BUY order is PARTIALLY_FILLED. Counter: ${partiallyFilledCounter.BUY}`,
            );
          } else {
            partiallyFilledCounter.BUY = 0;
          }

          if (partially_filled_status.has(orderTracker.sell.status)) {
            partiallyFilledCounter.SELL += 1;
            console.log(
              `SELL order is PARTIALLY_FILLED. Counter: ${partiallyFilledCounter.SELL}`,
            );
          } else {
            partiallyFilledCounter.SELL = 0;
          }

          if (
            partiallyFilledCounter.BUY >= 2 ||
            partiallyFilledCounter.SELL >= 2
          ) {
            console.log(
              'Order is PARTIALLY_FILLED for 2 consecutive iterations. Attempting to cancel both BUY and SELL orders...',
            );

            // Cancel BUY order
            try {
              if (buyApi instanceof Binance) {
                try {
                  await buyApi.cancelOrderBinance(
                    orderTracker.buy.id!,
                    config.symbols[config.buyExchange],
                  );
                } catch (error) {
                  console.error(`Error canceling BUY order: ${error}`);
                }
              } else if (buyApi instanceof Bybit) {
                try {
                  await buyApi.cancelOrder(
                    orderTracker.buy.id!,
                    config.symbols[config.buyExchange],
                  );
                } catch (error) {
                  console.error(`Error canceling BUY order: ${error}`);
                }
              } else if (buyApi instanceof Okx) {
                try {
                  const buyInstId = `${config.baseSymbol}-${config.assetForExchange2}-SWAP`;
                  await buyApi.cancelOrderById(orderTracker.buy.id!, buyInstId);
                } catch (error) {
                  console.error(`Error canceling BUY order: ${error}`);
                }
              }
            } catch (error) {
              console.error(`Error canceling BUY order: ${error}`);
            }

            // Cancel SELL order
            try {
              if (sellApi instanceof Binance) {
                try {
                  await sellApi.cancelOrderBinance(
                    orderTracker.sell.id!,
                    config.symbols[config.sellExchange],
                  );
                } catch (error) {
                  console.error(`Error canceling SELL order: ${error}`);
                }
              } else if (sellApi instanceof Bybit) {
                try {
                  await sellApi.cancelOrder(
                    orderTracker.sell.id!,
                    config.symbols[config.sellExchange],
                  );
                } catch (error) {
                  console.error(`Error canceling SELL order: ${error}`);
                }
              } else if (sellApi instanceof Okx) {
                try {
                  const sellInstId = `${config.baseSymbol}-${config.assetForExchange2}-SWAP`;
                  await sellApi.cancelOrderById(
                    orderTracker.sell.id!,
                    sellInstId,
                  );
                } catch (error) {
                  console.error(`Error canceling SELL order: ${error}`);
                }

                console.log('SELL order canceled successfully.');
              }
            } catch (error) {
              console.error(`Error canceling SELL order: ${error}`);
            }

            console.log('Skipping to next iteration...');
            count += 1;
            break;
          }

          //scenario 2  dealing with both fills:
          if (
            orderTracker.buy.status === filledStatus &&
            orderTracker.sell.status === filledStatus
          ) {
            count = count + 1;
            console.log('Both BUY and SELL orders are FILLED.');
            console.log(
              'New BUY order placed and tracker updated:',
              orderTracker,
            );
            break;
          }
          //scenario 3  dealing with both cancelled or expired:
          else if (
            cancelStatuses.has(orderTracker.buy.status) &&
            cancelStatuses.has(orderTracker.sell.status)
          ) {
            console.log(
              'Both the orders is canceled or expired. Exiting loop.',
            );
            console.log(
              'New BUY order placed and tracker updated:',
              orderTracker,
            );
            break;
          }

          //scenario 4  both are active:
          else if (
            activeStatuses.has(orderTracker.buy.status) &&
            activeStatuses.has(orderTracker.sell.status)
          ) {
            console.log(
              'Both BUY and SELL orders are in NEW status after 3 seconds. Canceling both orders...',
            );

            // Cancel BUY order
            try {
              if (buyApi instanceof Binance) {
                try {
                  await buyApi.cancelOrderBinance(
                    buyOrderId,
                    config.symbols[config.buyExchange],
                  );
                } catch (e) {
                  console.error(`Error canceling BUY order: ${e}`);
                }
              } else if (buyApi instanceof Bybit) {
                try {
                  await buyApi.cancelOrder(
                    buyOrderId,
                    config.symbols[config.buyExchange],
                  );
                } catch (e) {
                  console.error(`Error canceling BUY order: ${e}`);
                }
              } else if (buyApi instanceof Okx) {
                try {
                  const buyInstId = `${config.baseSymbol}-${config.assetForExchange2}-SWAP`;
                  await buyApi.cancelOrderById(buyOrderId, buyInstId);
                } catch (e) {
                  console.error(`Error canceling BUY order: ${e}`);
                }
              }
              console.log('BUY order canceled successfully.');
            } catch (e) {
              console.error(`Error canceling BUY order: ${e}`);
            }

            // Cancel SELL order
            try {
              if (sellApi instanceof Binance) {
                try {
                  await sellApi.cancelOrderBinance(
                    sellOrderId,
                    config.symbols[config.sellExchange],
                  );
                } catch (e) {
                  console.error(`Error canceling SELL order: ${e}`);
                }
              } else if (sellApi instanceof Bybit) {
                try {
                  await sellApi.cancelOrder(
                    sellOrderId,
                    config.symbols[config.sellExchange],
                  );
                } catch (e) {
                  console.error(`Error canceling SELL order: ${e}`);
                }
              } else if (sellApi instanceof Okx) {
                try {
                  const sellInstId = `${config.baseSymbol}-${config.assetForExchange2}-SWAP`;
                  await sellApi.cancelOrderById(sellOrderId, sellInstId);
                } catch (e) {
                  console.error(`Error canceling SELL order: ${e}`);
                }
              }
              console.log('SELL order canceled successfully.');
            } catch (e) {
              console.error(`Error canceling SELL order: ${e}`);
            }
          }

          //scenario 5  buy is active and sell is filled:
          else if (
            activeStatuses.has(orderTracker.buy.status) &&
            orderTracker.sell.status === filledStatus
          ) {
            console.log(
              'BUY order is NEW while SELL order is FILLED. Canceling BUY order and placing a new one...',
            );

            // Cancel the BUY order
            let cancelSuccessful = false;
            try {
              if (
                config.buyExchange === CryptoExchangeType.BINANCE &&
                buyApi instanceof Binance
              ) {
                try {
                  const cancelResponse = await buyApi.cancelOrderBinance(
                    orderTracker.buy.id!,
                    config.symbols[config.buyExchange],
                  );
                  if (isOrderCanceledBinance(cancelResponse)) {
                    console.log(
                      `BUY order ${orderTracker.buy.id} successfully canceled on Binance.`,
                    );
                    cancelSuccessful = true;
                  } 
                } catch (e) {console.error(`Error canceling BUY order: ${e}`);}
              } else if (
                config.buyExchange === CryptoExchangeType.BYBIT &&
                buyApi instanceof Bybit
              ) {
                try {
                  const cancelResponse = await buyApi.cancelOrder(
                    orderTracker.buy.id!,
                    config.symbols[config.buyExchange],
                  );
                  if (isOrderCanceledBybit(cancelResponse)) {
                    console.log(
                      `BUY order ${orderTracker.buy.id} successfully canceled on Bybit.`,
                    );
                    cancelSuccessful = true;
                  } 
                } catch (e) {
                  console.error(`Error canceling BUY order: ${e}`);
                }
              } else if (
                config.buyExchange === CryptoExchangeType.OKX &&
                buyApi instanceof Okx
              ) {
                try {
                  const buyInstId = `${config.baseSymbol}-${config.assetForExchange2}-SWAP`;
                  const cancelResponse = await buyApi.cancelOrderById(
                    orderTracker.buy.id!,
                    buyInstId,
                  );
                  if (isOrderCanceledOkx(cancelResponse)) {
                    console.log(
                      `BUY order ${orderTracker.buy.id} successfully canceled on OKX.`,
                    );
                    cancelSuccessful = true;
                  } 
                } catch (e) {console.error(`Error canceling BUY order: ${e}`);}
              }
            } catch (e) {
              console.error(`Error canceling BUY order: ${e}`);
            }

            // Place a new BUY order if cancellation was successful
            if (cancelSuccessful) {
              let newBuyOrderId: string | null = null;

              try {
                if (
                  config.buyExchange === CryptoExchangeType.BINANCE &&
                  buyApi instanceof Binance
                ) {
                  try {
                    const newBuyResponse = await buyApi.placeOrderBinance(
                      config.symbols[config.buyExchange],
                      'BUY',
                      config.minSlotQuantity,
                    );
                    newBuyOrderId = newBuyResponse?.clientOrderId || null;
                  } catch (e) {
                    console.error(`Error placing new BUY order: ${e}`);
                  }
                } else if (
                  config.buyExchange === CryptoExchangeType.BYBIT &&
                  buyApi instanceof Bybit
                ) {
                  try {
                    const newBuyResponse = await buyApi.placeLimitOrder(
                      config.symbols[config.buyExchange],
                      'Buy',
                      config.minSlotQuantity,
                      bybitPricePrecision,
                    );
                    newBuyOrderId = newBuyResponse?.result?.orderId || null;
                  } catch (e) {
                    console.error(`Error placing new BUY order: ${e}`);
                  }
                } else if (
                  config.buyExchange === CryptoExchangeType.OKX &&
                  buyApi instanceof Okx
                ) {
                  try {
                    const buyInstId = `${config.baseSymbol}-${config.assetForExchange2}-SWAP`;
                    const newBuyResponse = await buyApi.placeBboOrder(
                      buyInstId,
                      'buy',
                      config.minSlotQuantity,
                      okxContractVal,
                    );

                    newBuyOrderId = newBuyResponse.data[0]?.ordId || null;
                  } catch (e) {
                    console.error(`Error placing new BUY order: ${e}`);
                  }
                }

                if (newBuyOrderId) {
                  console.log(`New BUY order placed with ID: ${newBuyOrderId}`);
                  orderTracker.buy.id = newBuyOrderId;
                  orderTracker.buy.status = 'UNKNOWN'; // Reset status
                }
              } catch (e) {
                console.error(`Error placing new BUY order: ${e}`);
              }

              console.log(
                'Order Tracker updated with new BUY order:',
                orderTracker,
              );
            } else {
              console.log(
                'Cancellation failed. Skipping new BUY order placement.',
              );
            }
          }

          //scenario 6  buy is filled and sell is active:
          else if (
            activeStatuses.has(orderTracker.sell.status) &&
            orderTracker.buy.status === filledStatus
          ) {
            console.log(
              'SELL order is ACTIVE while BUY order is FILLED. Canceling SELL order and placing a new one...',
            );

            let cancelSuccessful = false;

            // Cancel the SELL order
            try {
              if (
                config.sellExchange === CryptoExchangeType.BINANCE &&
                sellApi instanceof Binance
              ) {
                try {
                  const cancelResponse = await sellApi.cancelOrderBinance(
                    orderTracker.sell.id!,
                    config.symbols[config.sellExchange],
                  );
                  if (isOrderCanceledBinance(cancelResponse)) {
                    console.log(
                      `Sell order ${orderTracker.sell.id} successfully canceled on Binance.`,
                    );
                    cancelSuccessful = true;
                  } 
                } catch (e) {
                  console.error(`Error canceling SELL order: ${e}`);
                }
              } else if (
                config.sellExchange === CryptoExchangeType.BYBIT &&
                sellApi instanceof Bybit
              ) {
                try {
                  const cancelResponse = await sellApi.cancelOrder(
                    orderTracker.sell.id!,
                    config.symbols[config.sellExchange],
                  );
                  if (isOrderCanceledBybit(cancelResponse)) {
                    console.log(
                      `Sell order ${orderTracker.sell.id} successfully canceled on Bybit.`,
                    );
                    cancelSuccessful = true;
                  } 
                } catch (e) {
                  console.error(`Error canceling SELL order: ${e}`);
                }
              } else if (
                config.sellExchange === CryptoExchangeType.OKX &&
                sellApi instanceof Okx
              ) {
                try {
                  const sellInstId = `${config.baseSymbol}-${config.assetForExchange2}-SWAP`;
                  const cancelResponse = await sellApi.cancelOrderById(
                    orderTracker.sell.id!,
                    sellInstId,
                  );
                  if (isOrderCanceledOkx(cancelResponse)) {
                    console.log(
                      `Sell order ${orderTracker.sell.id} successfully canceled on OKX.`,
                    );
                    cancelSuccessful = true;
                  } 
                } catch (e) {
                  console.error(`Error canceling SELL order: ${e}`);
                }
              }

              // Place a new SELL order
              if (cancelSuccessful) {
                try {
                  let newSellOrderId: string | null = null;
                  if (
                    config.sellExchange === CryptoExchangeType.BINANCE &&
                    sellApi instanceof Binance
                  ) {
                    try {
                      const newSellResponse = await sellApi.placeOrderBinance(
                        config.symbols[config.sellExchange],
                        'SELL',
                        orderQuantity,
                      );
                      newSellOrderId = newSellResponse?.clientOrderId || null;
                    } catch (e) {
                      console.error(`Error placing new SELL order: ${e}`);
                    }
                  } else if (
                    config.sellExchange === CryptoExchangeType.BYBIT &&
                    sellApi instanceof Bybit
                  ) {
                    try {
                      const newSellResponse = await sellApi.placeLimitOrder(
                        config.symbols[config.sellExchange],
                        'Sell',
                        orderQuantity,
                        bybitPricePrecision,
                      );
                      newSellOrderId = newSellResponse?.result?.orderId || null;
                    } catch (e) {
                      console.error(`Error placing new SELL order: ${e}`);
                    }
                  } else if (
                    config.sellExchange === CryptoExchangeType.OKX &&
                    sellApi instanceof Okx
                  ) {
                    try {
                      const sellInstId = `${config.baseSymbol}-${config.assetForExchange2}-SWAP`;
                      const newSellResponse = await sellApi.placeBboOrder(
                        sellInstId,
                        'sell',
                        orderQuantity,
                        okxContractVal,
                      );

                      newSellOrderId = newSellResponse.data[0]?.ordId || null;
                    } catch (e) {
                      console.error(`Error placing new SELL order: ${e}`);
                    }
                  }

                  // Update the order tracker with the new SELL order
                  if (newSellOrderId) {
                    orderTracker.sell.id = newSellOrderId;
                    orderTracker.sell.status = 'UNKNOWN';
                    console.log(
                      `New SELL order placed. Order ID: ${newSellOrderId}`,
                    );
                  } else {
                    console.log('Failed to place new SELL order.');
                  }
                } catch (e) {
                  console.error(`Error placing new SELL order: ${e}`);
                }
              } 
            } catch (e) {
              console.error(`Error: ${e}`);
            }
          }


          //scenario 7  buy is active and sell is cancelled:
          if (
            activeStatuses.has(orderTracker.buy.status) &&
            cancelStatuses.has(orderTracker.sell.status)
          ) {
            console.log(
              'BUY is ACTIVE and SELL is CANCELED. Canceling BUY order...',
            );

            // Cancel the BUY order
            try {
              if (
                config.buyExchange === CryptoExchangeType.BINANCE &&
                buyApi instanceof Binance
              ) {
                try {
                  await buyApi.cancelOrderBinance(
                    orderTracker.buy.id!,
                    config.symbols[config.buyExchange],
                  );
                  console.log('BUY order successfully canceled on Binance.');
                } catch (error) {
                  console.error(`Error canceling BUY order: ${error}`);
                }
              } else if (
                config.buyExchange === CryptoExchangeType.BYBIT &&
                buyApi instanceof Bybit
              ) {
                try {
                  await buyApi.cancelOrder(
                    orderTracker.buy.id!,
                    config.symbols[config.buyExchange],
                  );
                  console.log('BUY order successfully canceled on Bybit.');
                } catch (error) {
                  console.error(`Error canceling BUY order: ${error}`);
                }
              } else if (
                config.buyExchange === CryptoExchangeType.OKX &&
                buyApi instanceof Okx
              ) {
                try {
                  const buyInstId = `${config.baseSymbol}-${config.assetForExchange1}-SWAP`;
                  await buyApi.cancelOrderById(orderTracker.buy.id!, buyInstId);
                  console.log('BUY order successfully canceled on OKX.');
                } catch (error) {
                  console.error(`Error canceling BUY order: ${error}`);
                }
              }
            } catch (error) {
              console.error(`Error canceling BUY order: ${error}`);
            }

            console.log('BUY order canceled as SELL was already canceled.');
          }
          //scenario 8  buy is cancellled and sell is active:
          if (
            activeStatuses.has(orderTracker.sell.status) &&
            cancelStatuses.has(orderTracker.buy.status)
          ) {
            console.log(
              'SELL is ACTIVE and BUY is CANCELED. Canceling SELL order...',
            );

            // Cancel the SELL order
            try {
              if (
                config.sellExchange === CryptoExchangeType.BINANCE &&
                sellApi instanceof Binance
              ) {
                try {
                  await sellApi.cancelOrderBinance(
                    orderTracker.sell.id!,
                    config.symbols[config.sellExchange],
                  );
                  console.log('SELL order successfully canceled on Binance.');
                } catch (error) {}
              } else if (
                config.sellExchange === CryptoExchangeType.BYBIT &&
                sellApi instanceof Bybit
              ) {
                try {
                  await sellApi.cancelOrder(
                    orderTracker.sell.id!,
                    config.symbols[config.sellExchange],
                  );
                  console.log('SELL order successfully canceled on Bybit.');
                } catch (error) {
                  console.error(`Error canceling SELL order: ${error}`);
                }
              } else if (
                config.sellExchange === CryptoExchangeType.OKX &&
                sellApi instanceof Okx
              ) {
                try {
                  const sellInstId = `${config.baseSymbol}-${config.assetForExchange2}-SWAP`;
                  await sellApi.cancelOrderById(
                    orderTracker.sell.id!,
                    sellInstId,
                  );
                  console.log('SELL order successfully canceled on OKX.');
                } catch (error) {
                  console.error(`Error canceling SELL order: ${error}`);
                }
              }
            } catch (error) {
              console.error(`Error canceling SELL order: ${error}`);
            }

            console.log('SELL order canceled as BUY was already canceled.');
          }
          //scenario 9  buy is cancellled and sell is filled:
          if (
            cancelStatuses.has(orderTracker.buy.status) &&
            orderTracker.sell.status === filledStatus
          ) {
            console.log(
              'BUY is CANCELED and SELL is FILLED. Placing a new BUY order...',
            );

            let newBuyOrderId: string | null = null;

            // Place a new BUY order
            try {
              if (
                config.buyExchange === CryptoExchangeType.BINANCE &&
                buyApi instanceof Binance
              ) {
                try {
                  const buyResponse = await buyApi.placeOrderBinance(
                    config.symbols[config.buyExchange],
                    'BUY',
                    config.minSlotQuantity,
                  );
                  newBuyOrderId = buyResponse?.clientOrderId || null;
                  console.log(
                    `New BUY order placed on Binance. Order ID: ${newBuyOrderId}`,
                  );
                } catch (error) {
                  console.error(`Error placing BUY order: ${error}`);
                }
              } else if (
                config.buyExchange === CryptoExchangeType.BYBIT &&
                buyApi instanceof Bybit
              ) {
                try {
                  const buyResponse = await buyApi.placeLimitOrder(
                    config.symbols[config.buyExchange],
                    'Buy',
                    config.minSlotQuantity,
                    bybitPricePrecision,
                  );
                  newBuyOrderId = buyResponse?.result?.orderId || null;
                  console.log(
                    `New BUY order placed on Bybit. Order ID: ${newBuyOrderId}`,
                  );
                } catch (error) {
                  console.error(`Error placing BUY order: ${error}`);
                }
              } else if (
                config.buyExchange === CryptoExchangeType.OKX &&
                buyApi instanceof Okx
              ) {
                try {
                  const buyInstId = `${config.baseSymbol}-${config.assetForExchange1}-SWAP`;
                  const buyResponse = await buyApi.placeBboOrder(
                    buyInstId,
                    'buy',
                    config.minSlotQuantity,
                    okxContractVal,
                  );

                  newBuyOrderId = buyResponse.data[0]?.ordId || null;
                  console.log(
                    `New BUY order placed on OKX. Order ID: ${newBuyOrderId}`,
                  );
                } catch (error) {
                  console.error(`Error placing new BUY order: ${error}`);
                }
              }
            } catch (error) {
              console.error(`Error placing new BUY order: ${error}`);
            }

            // Update the tracker
            if (newBuyOrderId) {
              orderTracker.buy = { id: newBuyOrderId, status: 'UNKNOWN' };
              console.log(
                'New BUY order placed and tracker updated:',
                orderTracker,
              );
            } else {
              console.log('Failed to place a new BUY order.');
            }
          }
          //scenario 10  buy is filled and sell is cancelled:
          if (
            cancelStatuses.has(orderTracker.sell.status) &&
            orderTracker.buy.status === filledStatus
          ) {
            console.log(
              'SELL is CANCELED and BUY is FILLED. Placing a new SELL order...',
            );

            let newSellOrderId: string | null = null;

            // Place a new SELL order
            try {
              if (
                config.sellExchange === CryptoExchangeType.BINANCE &&
                sellApi instanceof Binance
              ) {
                try {
                  const sellResponse = await sellApi.placeOrderBinance(
                    config.symbols[config.sellExchange],
                    'SELL',
                    config.minSlotQuantity,
                  );
                  newSellOrderId = sellResponse?.clientOrderId || null;
                  console.log(
                    `New SELL order placed on Binance. Order ID: ${newSellOrderId}`,
                  );
                } catch (error) {
                  console.error(`Error placing SELL order: ${error}`);
                }
              } else if (
                config.sellExchange === CryptoExchangeType.BYBIT &&
                sellApi instanceof Bybit
              ) {
                try {
                  const sellResponse = await sellApi.placeLimitOrder(
                    config.symbols[config.sellExchange],
                    'Sell',
                    config.minSlotQuantity,
                    bybitPricePrecision,
                  );
                  newSellOrderId = sellResponse?.result?.orderId || null;
                  console.log(
                    `New SELL order placed on Bybit. Order ID: ${newSellOrderId}`,
                  );
                } catch (error) {
                  console.error(`Error placing SELL order: ${error}`);
                }
              } else if (
                config.sellExchange === CryptoExchangeType.OKX &&
                sellApi instanceof Okx
              ) {
                try {
                  const sellInstId = `${config.baseSymbol}-${config.assetForExchange2}-SWAP`;
                  const sellResponse = await sellApi.placeBboOrder(
                    sellInstId,
                    'sell',
                    config.minSlotQuantity,
                    okxContractVal,
                  );

                  newSellOrderId = sellResponse.data[0]?.ordId || null;
                  console.log(
                    `New SELL order placed on OKX. Order ID: ${newSellOrderId}`,
                  );
                } catch (error) {
                  console.error(`Error placing new SELL order: ${error}`);
                }
              }
            } catch (error) {
              console.error(`Error placing new SELL order: ${error}`);
            }

            // Update the tracker
            if (newSellOrderId) {
              orderTracker.sell = { id: newSellOrderId, status: 'UNKNOWN' };
              console.log(
                'New SELL order placed and tracker updated:',
                orderTracker,
              );
            } else {
              console.log('Failed to place a new SELL order.');
            }
          }

          console.log(
            'New BUY order placed and tracker updated:',
            orderTracker,
          );
        } catch (e) {
          console.error(`Error fetching order status: ${e}`);
        }
      }
    }


    // TODO: CHECK FOR QUANTITY FILLED CORRECT LOGIC
    return {
      jobStatus: JobStatus.COMPLETED,
      processedQuantity: count * config.minSlotQuantity * 2,
    };

  }
}
